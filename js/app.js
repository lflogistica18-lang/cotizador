...
function initApp() {
  ...
  document.getElementById('preciosInfo').textContent =
    'Precios: ' + precios.metadata.ultima_actualizacion + ' | ' + precios.metadata.moneda;
  document.getElementById('valorHoraAdmin').value =
    fmt(precios.mano_de_obra.valor_hora_administrativa);
  ...
  if ((Date.now() - updDate) / (1000 * 60 * 60 * 24) > 60) {
    ...
    aviso.textContent =
      'ATENCION: Los precios no se actualizan desde ' + precios.metadata.ultima_actualizacion;
